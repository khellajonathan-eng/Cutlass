import random, sys, time, ast, os, pickle, threading
from copy import deepcopy
v2 = "None" #x.x.V.x
v3 = "You can now make custom generation modes. Also added a new Extras class that has no relativity to the Cutlass engine, just extra features." #x.V.x.x
...
class EventHook(metaclass=EVMeta):
    ...
    onanyhook = defaultfunc#done
    @classmethod
    def createhook(cls, func, name, conds, *, endc=defaultfunc, passive=False):
        """Creates an attribute for a hook in the EventHook class."""
        if not name.startswith("on"):
            Error.error("Error 41: Name must start with on in order to show it is an event!")
            return
        if name in cls.defaults:
            Error.error("Error 43: Cannot change default hooks!")
            return
        if not callable(func):
            Error.error("Error 42: func parameter must be a function!")
            return
        setattr(cls, name, func)
        if passive:
            cls._activatehook(name, conds, end=endc)
    @classmethod
    def __manage(cls):
        """The manager command used for the thread"""
        while True:
            if cls.tickrate == 0:
                time.sleep(0.1)
                continue
            cls._events = {ev: co for ev, co in cls._events.items() if not isinstance(co, tuple) or not co[1]()}
            for event, conds in list(cls._events.items()):
                try:
            ...
            time.sleep(1 / cls.tickrate)
    @classmethod
    def initiate(cls):
        """Starts the manage command."""
        if not cls.__running:
            threading.Thread(target=cls.__manage, daemon=True).start()
            cls.__running = True
    @classmethod
    def _activatehook(cls, hook, cond, end=None):
        """"Turns a custom hook into a passive hook."""
        hook = getattr(cls, hook.__name__ if not isinstance(hook, str) else hook)
        if end is None:
            end = defaultfunc
        if hook in cls.defaults.values() or hook in cls.defaults.keys():
            Error.error("Error 43: Cannot change default hooks!")
        if end != cls.defaultfunc:
            cls._events[hook] = (cond, end)
        else:
            cls._events[hook] = cond
EventHook.defaults = {name: func for name, func in EventHook.__dict__.items() if name.startswith("on")}
class Error:
    """The Error/Warning system."""
    @staticmethod
    def error(msg, errtype="normal", exc=False, sourcefile=None, finderror=False, debug=False):
        """Creates a custom error, which if you use exc, it would raise a custom error with parent exception Panic."""
        #exc just kills the system
        ...
        try:
            ...
            if not exc:
                print((f"\033[32m{errtype} " if errtype != "normal" else "") + ("\033[31m" if exc else "") + f"{msg}\033[0m", (f"\033[96merror at {lineno}\033[0m" if finderror else ""))
            else:
                if errtype != "normal":
...
class Map:
                ...
                for m in mapss:
                    if m == ans:
                        ...
                        if player.rlocation is None:
                            Error.error("\033[31mFatal Error 404: nonexistent room detected. restart the script.\033[0m", exc=True)
                return
    ...
    def generaterooms(self, num=2, *ro, debug=False, hidden=None, showit=False, loot=None, needs=None, buys=None, truehide=None, mode="default"):
        ...
        for r in ro:
            if r in nocs or len(set(ro)) != len(ro):
        ...
        if mode != "default":#dev note: this took so long i swear it crashed at every nook and cranny finally fixed it tho
            ...
            if basemode == "tree":
                ...
                if len(nosrobots) > num:
                    ...
                    Error.error("Error 37: Number of exits cannot be more than or equal to the amount of rooms provided!")
                    return
            elif basemode in Map.custommodes.keys():
                try:
                    Map.custommodes[basemode](nosrobots, num)
                except Exception:
                    del Map.custommodes[basemode]
            else:
...
class System:
    ...
    def credits(cls):
        ...
        print("PS from dev: i am lonely :(. ALSO WHY ARE THERE SO MANY BUGS IN DEMO AHHH")
        ...
        print("6: Only ever activate a custom hook if you dont intend on calling it yourself!")
    @classmethod
    def radar(cls):
    ...
    def kill(cls, check, name):
        ...
        if check == "m":
            if name in Map.mapsnames.keys():
                for r in list(Map.mapsnames[name].lor):
                    cls.kill("r", r.name)
    ...
    def run(self):
        ...
        while self.running:
            ...
            match command.split():
                case ["help"]:
                    print("=== HELP ===")
                    print("generate (optional) [mode] [options]")
                    print("raise [error]")
                    print("credits")
                    print("rooms")
                    print("maps")
                    ...
                    print("move [where]")
                    print("move interactive")
                    print("clear")
                    print("quit")
                case ["generate", *options]:
                    if "TestMap" in Map.mapsnames:
                        System.kill("m", "TestMap")
                    testmap = Map("TestMap")
                    result = options[0] if options else "default"
                    testmap.generaterooms(mode=f"{result}{'/' if options else ''}{'/'.join(options) if options else ''}")
                case ["raise", *errors]:
                ...
                case ["inspect", *target]:
                    target = " ".join(target)
                    f = False
                    def getdict(obj, filter=lambda n: not n.startswith("__")):
                        attrdict = {}
                    ...
                    for r in Map.Room.rooms:
                        if r.name == target:
                            print("If any, Temporary Error 0 is when something went wrong with getting the attribute.")
                            attrs = getdict(r)
                            f = True
                            break
                    if not f:
                        if target == "player":
                            attrs = getdict(player)
                            f = True
                        elif hasattr(EventHook, target):
                            attrs = {target: getattr(EventHook, target, "Temporary Error 0")}
                            f = True
                        elif target == "hooks":
                            attrs = getdict(EventHook, lambda n: n.startswith("on"))
                            f = True
                        elif target in globals().keys():
                            attrs = {target: globals()[target]}
                            f = True
                    if f:
                        for k, v in attrs.items():
                            print(f"{repr(k)} = {repr(v)}")
                    else:
                        Error.error("Error 404j: Object not found")
                case ["move", "interactive"]:
                    Map.Room.diffroom()
                    defaulted = True
                case ["move", *target]:
                    target = " ".join(target)
...
class Extras:
    """The class for extra, unrelated features that I made which I thought were really cool.."""
    @staticmethod
    def sine(mx, mn, *, travel=0.05, roundby=3, speed=0.01, dist=3, times=100, surge=0, shift=0, thrust=1, flux=1, resetforce=False, show=False):
        """Makes a form of a sine wave."""
        current = mx + mn
        force = 0
        form = 1
        assert -abs(mn) == mn and abs(mx) == mx, "Maximum value must be a positive number and minimum value must be negative!"
        clock = 0
        while clock != times:
            if show:
                print(current, end="\r")
            ...
            if current <= mn:
                ...
                if resetforce:
                    force = 0
            thrust = -abs(thrust ** flux) if form == 0 else abs(thrust ** flux)
            shift *= thrust
            surge += shift
            travel += surge
            force += travel
            current += force
            thrust = abs(thrust) if form == 1 else -abs(thrust)
            travel = abs(travel) if form == 1 else -abs(travel)
            surge = abs(surge) if form == 1 else -abs(surge)
            shift = abs(shift) if form == 1 else -abs(shift)
            time.sleep(speed)
            current = round(current, roundby)
            clock += 1
            yield current