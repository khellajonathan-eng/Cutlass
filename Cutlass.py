...
v2 = "Fixed ANOTHER a bug in Demo() which prevents users from running generate default." #x.x.V.x
...
class Map:
    ...
    def generaterooms(self, num=2, *ro, debug=False, hidden=None, showit=False, loot=None, needs=None, buys=None, truehide=None, mode="default"):
        ...
        if mode != "default" and not mode.endswith("/"):#dev note: this took so long i swear it crashed at every nook and cranny finally fixed it tho
            print("thing")
            if not roms:
                Error.error("Error 6: No rooms detected")
                return
        ...
        while self.running:
            ...
            match command.split():
                ...
                case ["generate", *options]:
                    testmap = Map("TestMap")
                    testmap.generaterooms(mode=f"{options[0] if options else 'default'}/{'/'.join(options[1:])}")
                    print(repr(options[0] if options else "default"))
                case ["raise", *errors]:
