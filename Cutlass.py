...
v2 = "Fixed YET ANOTHER bug in Demo() which creates duplicate rooms if the generate command is used twice." #x.x.V.x
v3 = "Updated EventHook which now has createhook() and initiate()." #x.V.x.x
...
class EventHook(metaclass=EVMeta):
    ...
    def createhook(cls, func, name, conds, *, endc=defaultfunc, passive=False):
        ...
        if passive:
            r = cls._activatehook(name, conds, end=endc)
    @classmethod
    def __manage(cls):
