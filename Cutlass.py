...
v2 = "Fixed a bug in Demo() which prevents users from running generate default. The generate command also works by itself now." #x.x.V.x
        ...
        while self.running:
            ...
            match command.split():
                ...
                case ["generate", *options]:
                    testmap = Map("TestMap")
                    testmap.generaterooms(mode=f"{options[0]}/{'/'.join(options[1:])}")
                    print(repr(options[0]))
                case ["raise", *errors]:
